...
(async () => {
  ...
  const teacherMap = new Map(); // teacherId -> Set of teacher names
    ...
    const courses = await listPage.evaluate(() => {
      ...
      for (const row of rows) {
        ...
        if (cells.length >= 6) {
          ...
          if (link) {
            // Teacher name is typically in the 6th column (index 5)
            // Column structure: 开课院系, 课程号, 课序号, 课程名, 学分, 主讲教师, ...
            const teacherCell = cells[5]; // 主讲教师 column
